...
# routes['GET']['/courses/destroy/<int:course_id>'] = 'Courses#show'
...
class Courses(Controller):
    ...
    def show(self, course_id):
        ...
        print "show!"
        return self.load_view('success.html', course=course)
    ...
    def add(self):
        ...
        # from a form on our client
            if not title:
                error = True
                flash("Cannot leave blank", "error")
            if not len(request.form['title']) < 15:
                error = True
                flash("Course name must be at least 15 characters long!", "error")
            if not error: 
                print "add working"
                course_details = {
                    'title': request.form['title'],
